 ...
 *     단 select 태그의 selectIndex나 value 변경 시 onchange 발생 안하므로 change 이벤트 발생시켜 줘야 함
 ...
 *       };
 *   
 *       var selectBox = new Hanatour.components.selectbox('div.select', options); // select 태그 기반이 아닌 경우엔 div.select 를 target으로 삼는다.
 * 
 *       $(function () {
 *           selectBox.init();  
 *       });
 ...
 *           aClass: 'on',                  //리스트 LI의 A 태그의 오버,아웃,활성화와 div.select > a 활성화를 담당할 클래스 명
 *           divClass: 'select',            //컨테이너 DIV 태그 클래스 명
 *           btnClass: 'tit',               //클릭할 A 태그의 클래스명
 *           conClass: 'overcon',           //UL 감싸고 있는 DIV 클래스명
 *           ulClass: 'con',                //UL 클래스명
 *			 chooseClass: undefined,	    //선택과 미선택 구분 클래스
 *           disabledClass: undefined,      //셀렉트 박스 비활성화 클래스
 *           disabledItemClass: 'disabled'  //리스트 아이템 비활성화 클래스
 ...
 *           autoListWidth: false,          //리스트 크기가 가장 긴 텍스트에 맞게 자동 설정
 ...
 * 
 *       .getTarget();             //div 타깃 반환 (div.select)
 *       .getInfo();               //정보반환, change 이벤트의 param 와 같음
 *       .setIndex(idx);           //index로 선택
 *       .getIndex();              //index 반환
 *       .setValue(value);         //value로 선택
 *       .getValue()               //value 반환
 *       .setText(value);          //text로 선택
 *       .getText();               //text 반환
 *       .setDisabled(boolean);    //셀렉트 박스 disabled
 *       .setReset();              //select 태그를 토대로 재적용
 *       .init();                  //디자인 셀렉트 생성
 *       .dispose();               //디자인 셀렉트 삭제
 *
 * 		//기본 셀렉트 박스
...
var SelectBox = (function ($) {
    var wddoObj = function (_target, _options) {
        ...
        function createLayout() {
            if ($target.is('select')) {
                ...
                $hiddenSelect.find('option').each(function () {
                    ...
                    li.filter(function () {
                    	return optDisabled
                    }).hide().addClass(opts.disabledItemClass); //modify 1.5.9
        ...
        function initLayout() {
            ...
            listHeight = ($listUL.children().not('.disabled').length * itemHeight) + bhListUL; //$listUL.outerHeight(); crossbrowser error
        ...
        function initEvent() {
            ...
            if (isValue) {
                $btnA.add($listA).on('keydown.selectbox', function (e) {
                    ...
                    //alt open, not search keyword space
                    if (altOpen && !isOpen || keyCode === 32 && previousFilter === undefined) {
                        ...
                        switch (keyCode) {
                            ...
                            case 9 : //tab, is open
                                ...
                                break;
                            case 35 : //end, pagedown
                            case 34 :
                                selectList(li.not('.disabled').filter(':last').find('a'));

                                e.preventDefault();
                                break;
                            case 40 : //down, right
                            case 39 :
                                if (scope.getIndex() !== undefined) {//not label
                                    var nextLI = li.eq(idx).nextAll().not('.disabled').filter(':first');
                            ...
                            case 36 : //home, pageup
                            case 33 :
                                selectList(li.not('.disabled').filter(':first').find('a'));

                                e.preventDefault();
                                break;
                            case 38 : //up, left
                            case 37 :
                                var prevLI = li.eq(idx).prevAll().not('.disabled').filter(':first');
