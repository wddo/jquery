 ...
 * 2.2.3 (2015.09.25) : isWait 옵션 추가, true 시 어떤 버튼 클릭하더라도 반응 하지 않는다.
 * 2.2.4 (2015.11.13) : dispose() 다중 호출시 if ($container === undefined) return; 로 막음
 *                      jump 가 1 이상일때 min, max 를 벋어남 수정
 *                      limit 변수와 checkLimit() 추가
 *                      exDot 를 exChange 로 변경하고 파라미터로 direct 넘겨 방향 확인 가능, exUp, exDown 삭제
 * 2.2.5 (2015.12.14) : 2.2.4 에서 비공식 수정한 rollingData 옵션에서 객체들 jQuery Object로 받는 것을 다시 문자열로 받는걸로 수정 (이유: resetItem() 에서 item을 재정의 할 방법이 없음)
 *                      2.2.4 에서 비공식 수정한 (count > min) -> (count - jump > min) 을 (count - (jump-1) > min) 로 변경하여 jump = 1 에는 기존 동작대로 되도록 수정(currentIdx, count 영향 안받게)
 * 2.2.6 (2016.01.08) : resetItem() 수정하여 2.2.5 돌려놓은 rollingData 다시 jQuery Object 로 받을 수 있게 수정
 *                      motionComplete 에 sortItem() 실행 여부 수정
 * 
...
*     wait: false,                              //이동 중 이동 막을지 유무            기본값 false
...
var Rolling = (function ($) {
    var wddoObj = function () {
        ...
        function init(data) {
            dataDefault = data;
            $container = (typeof data.item === 'string') ? $(data.container) : data.container;
            $itemContainer = (typeof data.itemContainer === 'string') ? $(data.itemContainer) : data.itemContainer;
            $item = (typeof data.item === 'string') ? $(data.item) : data.item;
            $upA = (typeof data.upA === 'string') ? $(data.upA) : data.upA;
            $downA = (typeof data.downA === 'string') ? $(data.downA) : data.downA;
            $dotA = (typeof data.dotA === 'string') ? $(data.dotA) : data.dotA;
            range = data.range || 1;
            jump = data.jump || 1;
            ...
            isWait = (data.wait !== undefined) ? data.wait : false;
        ...
        function up() {
            if (isWait && $itemContainer.filter(':animated').length > 0) return;
            ...
            if (isRepeat) {
                ...
                checkLimit();
                currentIdx = count = (count - (jump-1) > min) ? count - jump : (isFirst) ? max : min; //modify 2.2.4, (count > min) -> (count - (jump-1) > min)
            }
        ...
        function down() {
            ...
            if (isRepeat) {
                ...
                //일반형, 양끝에선 정지
                currentIdx = count = (max > count + (jump-1)) ? count + jump : (isFirst) ? min : max; //modify 2.2.4, (max > count) -> (max > count + (jump-1))
            }
        ...
        function dot(idx, spd) {
            ...
            currentIdx = count = idx;

            if (!isRepeat) checkLimit();
        ...
        //아이템 리스트 리셋
        function resetItem() {
            $item = (typeof dataDefault.item === 'string') ? $(dataDefault.item) : $(dataDefault.item.context).find(dataDefault.item.selector);
        ...
        function checkLimit() {
            ...
            if (!(max > count + jump) && direct === 'down') {
                //다음 버튼으로 이동 시
                limit = 'last';
            } else if (!(count - jump > min) && direct === 'up') {
                //이전 버튼으로 이동 시 
                limit = 'first';
                ...
                if (count <= 0) {
                    limit = 'first';
                } else if (count + jump > itemTotal - 1) {
                    limit = 'last';
                } else {
                    limit = undefined;
                }
        ...
        function motionComplete () {
            hideAtag();
            if (range === jump && !isRepeat) sortItem(); //1개이상 아이템이 보일 때도 repeat 가능하므로 sort 시키지 않음 //modify 2.2.6
