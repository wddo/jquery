 ...
 * 4.1.0 (2020.05.09) : jQuery 3.x.x 에서 animate의 speed 0일때 complete 콜백 실행 안되는 문제 해결, resetItem() 내 .context, .selector 미지원 대응
 * 
 ...
 * 
 * var rollingData = {
 *     container: '.ir_data > dl > dd',          //전체 영역 부모 jquery 셀렉터
 *     itemContainer: '.ir_data .con01 > ul',    //리스트를 감싸고 있는 부모 jquery 셀렉터
 *     item: '.ir_data .con01 > ul > li',        //리스트들 jquery 셀렉터
 *     controller : '.ir_data .btn',             //컨트로러 jquery 셀렉터
 *     prevA: '.ir_data .btn > a:eq(1)',         //화살표 이전 jquery 셀렉터
 *     nextA: '.ir_data .btn > a:eq(0)',         //화살표 다음 jquery 셀렉터
 *     dotA: '.ir_data .dot > a',                //도트버튼 jquery 셀렉터 | html 태그
 *     distance: 18,                             //클릭 시 이동할 거리
 *     range: 1,                                 //노출될 리스트 갯수
 *     jump: 1,                                  //한번에 이동 할 갯수
 *     delay: 5000,                              //자동롤링 시간
 *     speed: 800,                               //모션 속도
 ...
 *
 *   rolling.dispose()               //파괴
 *   rolling.setRefresh(idx)         //새로고침. setRest() 포함하고 있으며 현재상태 유지용 //idx: 해당 idx로 초기화
 *   rolling.setReset()              //리셋
 *   rolling.setChange(idx, spd);    //리스트 변경 //idx: 변경할 인덱스, spd: 변경 속도. 미지정시 옵션 속도 
 *   rolling.setPlay();              //자동롤링 활성화
 *   rolling.setStop();              //자동롤링 비활성화
 *   rolling.setNext(spd);           //다음으로 이동
 *   rolling.setprev(spd);           //이전으로 이동
 *   rolling.setJump(value);         //점프 옵션 변경
 *   rolling.getCurrent();           //현재 인덱스 반환
 *   rolling.getTotal();             //총 갯수 반환
    ...
    var wddoObj = function () {
        ...
        function resetItem() {
            ...
            if (typeof opts.item === 'string') {
                ...
                var $context = typeof opts.item.context === 'undefined' ? $itemContainer : $(opts.item.context);
                var selector = typeof opts.item.selector === 'undefined' && $item.length > 0 ? '>' + $item.get(0).tagName : opts.item.selector;
        ...
        function motion(pos, spd) {
            ...
            var st = (opts.direction === 'horizontal') ? {'marginLeft': pos + "px"} : {'marginTop': pos + "px"};
            $itemContainer.stop().filter(function () {
                if (sp === 0) {
                    $(this).css(st);
                    motionComplete();
                }
                return sp !== 0
            }).animate(st, {queue:false, duration:sp, easing: opts.ease, complete: motionComplete});  
        }
        ...
        this.dispose = function () {
            ...
            $item.find('a').removeData('idx').css('visibility', '');
                if ($prevA !== undefined) $prevA.off('.rolling');
                if ($nextA !== undefined) $nextA.off('.rolling');
                if ($dotA !== undefined) $dotA.off('.rolling').removeData('idx');
