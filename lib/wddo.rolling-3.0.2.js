...
var Rolling = (function ($) {
    var wddoObj = function () {
        ...
        function init() {
            ...
            //외부 확장 함수 호출
            if (opts.onInit !== undefined) opts.onInit({idx: currentIdx, container: $container, activate: $activateItem, total: itemTotal, limit: limit, direct: direct});
        }

        //dotA 초기화
        function initDotA() {
            //태그형태로 넘어 왔을 경우 복제할 dotA 생성
            if (typeof opts.dotA === 'string' && opts.dotA.indexOf('<') === 0) {
                opts.dynamicDot = true;
                $container.append(opts.dotA);
            ...
            //$dotA 초기화
            $dotA = ($dotA.is('a') && $dotA.parent().is('li, span')) ? dotList.children() : dotList;
        }
        ...
        function prev() {
            ...
            //외부 확장 함수 호출
            if (opts.onChange !== undefined) opts.onChange({idx: currentIdx, container: $container, activate: $activateItem, total: itemTotal, limit: limit, direct: direct});
        }
        ...
        //아이템 리스트 리셋
        function resetItem() {
            //.children() 같은 메소드 지원하지 않으니 find 로만 찾을 것
        ...
        function switchItem(goIdx) {
            ...
            if (direct === 'prev') {
                posNum = (opts.repeat) ? opts.range : Math.min(Math.abs(currentIdx - goIdx), opts.range); //현재위치에서 목적 아이템 위치를 빼면 이동 거리가 나오는데 range 를 넘어서면 range 까지만 이동
        ...
        function fadeIn() {
            ...
            if (opts.effect === 'fade') {
                target.filter(function (idx) {
                    ...
                    'opacity' : 1
                }, {queue:false, duration:opts.speed, complete: function () {
        ...
        function complete() {
            ...
            if (opts.onComplete !== undefined) opts.onComplete({idx: currentIdx, container: $container, activate: $activateItem, total: itemTotal, limit: limit, direct: direct});
        }
