 ...
 *                      initDotA() 에서 hide() 하던 dotA 를 remove()로 완전 삭제하도록 수정, 1개의 dotA 만 남으면 숨김
 *                      
 *                       
 * 
...
var Rolling = (function ($) {
    var wddoObj = function () {
        ...
        //dotA 초기화
        function initDotA() {
            var dotList = ($dotA.is('a') && $dotA.parent().is('li')) ? $dotA.parent() : $dotA;
            ...
            if (opts.dynamicDot && $dotA.length > 0) {
                ...
                //$dotA 초기화
                $dotA = ($dotA.is('a') && $dotA.parent().is('li')) ? dotList.children() : dotList;
            }

            //리스트 개수가 jump 와 맞지 않고 반복이면 dotA 모두 숨김
            ...
            dotList.each(function (idx) {
                if (0 !== idx % opts.jump) $(this).remove();
            });
