 ...
 * 3.1.1 (2016.10.20) : opts.autoArrowView 추가하여 prevA, nextA 의 show(), hide() 확장 수정 가능토록 수정
 * 3.1.2 (2016.10.26) : setPrev(), setNext() 로직 변경, getDirect() 추가
 * 3.1.3 (2017.02.16) : lazyload 적용 initLazy(), restartLazy(), opts.lazy 추가, lazy옵션인 lazyOpts, lazyDefaults 변수 추가하고 init() 두번째 인자로 받음
 * 3.1.4 (2017.04.13) : opts.poistion 추가하여 fade 효과 모션 완료 일때 css position값 조정 가능토록 수정
 * 3.1.5 (2017.04.20) : changeArrowBtn() 에서 page넘버가 아닌 currentIdx 와 itemTotal 로 체크, dot()에 대한 연속호출 대응(setNext(), setPrev() 에서 repeat:false 시 dot() 호출)
 * 
...
var WRolling = (function ($) {
    var wddoObj = function () {
        var scope,
            ...
            lazyOpts,
            lazyDefaults = lazyDefaultOptions(), //lazy 기본값 //add 3.1.3
            isPlay,
            distance,
            itemTotal,
            posNum = 0,
        ...
        function prev(spd) {
            ...
            motion(0, spd); //margin 을 0 으로 모션이동
        ...
        function next(spd) {
            ...
            //모션 시작
            motion(-distance * posNum, spd); //margin 을 distance 만큼 왼쪽으로 모션 이동
        ...
        function dot(idx, spd) {
            ...
            if ($itemContainer.filter(':animated').length > 0) motionComplete(); //연속 호출 시 어그러짐으로 선행 //add 3.1.5
        ...
        function resetItem() {
            ...
            $item = (typeof opts.item === 'string') ? $(opts.item) : $(opts.item.context).find(opts.item.selector); //modify 3.1.3 defaults -> opts
        ...
        function complete() {
            saveActivate();
            restartLazy(); //lazy 재로드 //add 3.1.3
        ...
        function changeArrowBtn() {
            if (opts.autoArrowView && !opts.repeat && $prevA !== undefined && $nextA !== undefined) {
                if ($prevA.length > 0 && $nextA.length > 0) {
                    ...
                    //modify 3.1.5 ..page 가 아닌 idx 로 체크
                    /*if (currentPage === 1) */ if (currentIdx <= 0) $prevA.hide();
                    /*if (currentPage === totalPage)*/ if (itemTotal - opts.range <= currentIdx) $nextA.hide();
                }
            }
        }
        ...
        //lazyload() 내부 $.rightoffold()에 의한 미로드 item들에 대하여.. //화면에 보여지면 로드되지 못한 lazy 재로드 신청 //add 3.1.3
        ...
        this.init = function (options, lazyOptions) {
            opts = $.extend({}, defaults, options);  //({}, 추가 modify 3.1.3 
            lazyOpts = $.extend({}, lazyDefaults, lazyOptions); //add 3.1.3
        ...
        this.dispose = function () {
            ...
            lazyOpts = undefined; //add 3.1.3
            lazyDefaults = lazyDefaultOptions(); //add 3.1.3
            isPlay = undefined;
            distance = undefined;
            itemTotal = undefined;
            posNum = 0;
